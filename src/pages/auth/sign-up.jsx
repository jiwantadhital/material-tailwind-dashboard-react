...
import { Eye, EyeOff, User, Phone, Lock, CheckCircle, Shield, ArrowLeft } from 'lucide-react';
...
export function SignUp() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.success) {
        // Clear the token since we want user to verify phone first
  ...
  const handleOtpSubmit = async (e) => {
    ...
    try {
      const response = await authService.verifyPhoneNumber(otpData.phone, otpData.otp);
      ...
      if (response.success) {
        ...
        navigate('/auth/sign-in', { 
          replace: true,
          state: { 
            message: 'Phone number verified successfully! Please sign in with your credentials.',
            type: 'success'
          }
        });
      } else {
  ...
  const handleResendOtp = async () => {
    setOtpData({ ...otpData, resendLoading: true });
    setError('');
    
    try {
      const response = await authService.resendPhoneOtp(otpData.phone);
      ...
      setOtpData({ ...otpData, resendLoading: false });
    }
  };
  ...
  const handleBackToRegistration = () => {
    ...
    setOtpData({ otp: '', phone: '', resendLoading: false });
    setError('');
  };

  // Define registration form content
    ...
    <form onSubmit={handleOtpSubmit} className="space-y-6">
      ...
      <div className="text-center space-y-3">
        ...
        <Button 
          ...
          color="blue" 
          size="sm"
          disabled={otpData.resendLoading}
          onClick={handleResendOtp}
          className="font-semibold"
        >
          {otpData.resendLoading ? 'Sending...' : 'Resend Code'}
        </Button>
      </div>
        ...
        {currentStep === 'registration' ? (
          ...
          <>
            <div className="text-center mb-8">
              ...
              <Typography variant="paragraph" color="blue-gray" className="text-blue-gray-500 mt-1">
                We've sent a 6-digit code to {otpData.phone}
              </Typography>
            </div>
            {otpFormContent}
          </>
